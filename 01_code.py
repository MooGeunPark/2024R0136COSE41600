...
# PCD 파일이 있는 폴더 경로 설정
folder_path = "/Users/moogeunpark/Desktop/college/code/자율주행/data/01_straight_walk/pcd"
    ...
    while idx < len(pcd_files) and len(frame_images) < max_frames:  # 최대 프레임수 제한
        ...
        for i in range(labels.max() + 1):
            ...
            if min_points_in_cluster <= len(cluster_indices) <= max_points_in_cluster:
                ...
                if (
                    min_z_value <= z_min <= max_z_value
                    and min_height <= height_diff <= max_height
                    and distances.max() <= max_distance
                ):
        ...
        ctr.set_front([0, -2, 1])  # front 방향을 45도 기울이기
